...
@Component({
        ...
        <article class="panel journey-panel">
          ...
          <div class="journey" *ngIf="timeline.length; else emptyJourney">
            <div class="journey-step" *ngFor="let item of timeline; let last=last" [class.last]="last">
              ...
              <div class="step-card">
                <div><span class="activity-type">{{activityLabel(item)}}</span><small>{{item.salesExecutive}}</small></div>
  ...
  `,
  styles: [`
    .back-link{display:inline-flex;align-items:center;text-decoration:none}.loading{padding:40px;text-align:center;color:var(--muted)}.profile-hero{padding:24px;margin-bottom:20px;border-radius:18px;background:linear-gradient(125deg,#0f172a,#134e4a);color:white;display:flex;justify-content:space-between;align-items:center;gap:24px;box-shadow:0 16px 34px rgba(15,23,42,.18)}.identity{display:flex;align-items:center;gap:16px}.avatar{width:64px;height:64px;border-radius:18px;display:grid;place-items:center;background:linear-gradient(135deg,#2dd4bf,#0f766e);font-size:22px;font-weight:900}.source-badge{display:inline-block;padding:4px 8px;margin-bottom:6px;border-radius:99px;background:rgba(255,255,255,.12);color:#99f6e4;font-size:10px;font-weight:800;text-transform:uppercase;letter-spacing:.6px}.identity h2{margin:0 0 4px;font-size:25px;color:white}.identity p{margin:0;color:#cbd5e1}.hero-facts{display:grid;grid-template-columns:repeat(4,minmax(110px,1fr));gap:10px}.hero-facts div{padding:12px;border:1px solid rgba(255,255,255,.12);border-radius:12px;background:rgba(255,255,255,.06)}.hero-facts span,.hero-facts strong{display:block}.hero-facts span{font-size:10px;color:#94a3b8;margin-bottom:4px;text-transform:uppercase}.hero-facts strong{font-size:13px;color:white}.profile-layout{display:grid;grid-template-columns:minmax(0,1.05fr) minmax(380px,.95fr);gap:20px;align-items:start}.editor,.journey-panel{padding:22px}.section-head{display:flex;justify-content:space-between;align-items:flex-start;gap:15px;margin-bottom:20px}.section-head h2{margin:2px 0 0}.section-head p:not(.eyebrow){margin:5px 0 0;color:var(--muted);font-size:12px}.saved{padding:6px 10px;border-radius:99px;background:#ecfdf3;color:#047857;font-size:11px;font-weight:800}.form-grid{display:grid;grid-template-columns:1fr 1fr;gap:14px}.form-grid .full{grid-column:1/-1}.form-actions{display:flex;align-items:center;justify-content:space-between;gap:14px;margin-top:20px;padding-top:16px;border-top:1px solid var(--line)}.form-actions span{font-size:11px;color:var(--muted)}.journey-count{white-space:nowrap;padding:6px 10px;border-radius:99px;background:#e8f5f3;color:#0f766e;font-size:11px;font-weight:800}.journey-summary{display:grid;grid-template-columns:1fr 1fr;gap:10px;margin-bottom:18px}.journey-summary div{padding:10px;border-radius:10px;background:#f8fafc;border:1px solid var(--line)}.journey-summary span,.journey-summary strong{display:block}.journey-summary span{font-size:10px;color:var(--muted);margin-bottom:3px}.journey-summary strong{font-size:12px}.journey-step{display:grid;grid-template-columns:28px 1fr;gap:9px;min-height:48px}.rail{display:flex;align-items:center;flex-direction:column}.point{position:relative;z-index:1;width:24px;height:24px;border-radius:50%;display:grid;place-items:center;background:#0f766e;color:white;font-size:10px;font-weight:900;box-shadow:0 0 0 4px #e8f5f3}.point.visit{background:#7c3aed;box-shadow:0 0 0 4px #f3e8ff}.rail i{width:2px;flex:1;min-height:18px;background:#cbd5e1}.journey-step.last .rail i{background:linear-gradient(#cbd5e1,transparent)}.step-card{display:flex;align-items:flex-start;justify-content:space-between;gap:12px;padding:1px 10px 13px 0;border-bottom:1px solid #f1f5f9}.step-card div{min-width:0}.activity-type,.step-card small{display:block}.activity-type{font-size:12px;font-weight:900;color:#0f766e}.step-card small{margin-top:2px;color:#64748b;font-size:10px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.step-card time{white-space:nowrap;color:var(--muted);font-size:10px}.current-stage{display:flex;align-items:center;gap:12px;margin-left:2px;margin-top:4px;padding:10px;border-radius:12px;background:#fffbeb;border:1px solid #fde68a}.finish-point{width:26px;height:26px;border-radius:50%;display:grid;place-items:center;background:#f59e0b;color:white}.current-stage small,.current-stage strong{display:block}.current-stage small{font-size:10px;color:#92400e}.current-stage strong{color:#78350f;font-size:13px}.empty-journey{text-align:center;padding:28px 10px;color:var(--muted)}.empty-journey>span{display:block;font-size:30px}.empty-journey strong{display:block;color:#334155;margin:7px}.empty-journey p{margin:0;font-size:12px}@media(max-width:1100px){.profile-hero{align-items:flex-start;flex-direction:column}.hero-facts{width:100%}.profile-layout{grid-template-columns:1fr}}@media(max-width:680px){.hero-facts{grid-template-columns:1fr 1fr}.form-grid{grid-template-columns:1fr}.form-grid .full{grid-column:auto}.profile-hero{padding:18px}.identity{align-items:flex-start}.avatar{width:52px;height:52px}.journey-summary{grid-template-columns:1fr}.step-card{align-items:flex-start;flex-direction:column;gap:3px}.step-card time{white-space:normal}}
  `]
})
export class LeadProfileComponent implements OnInit {
  ...
  activityLabel(item:FollowUp){if(item.resultingStatus!==null&&item.resultingStatus!==undefined)return this.statusLabel(item.resultingStatus);return item.type===5?'Site Visit':'Follow-up'}
