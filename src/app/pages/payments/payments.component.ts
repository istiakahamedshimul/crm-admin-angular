...
@Component({
  ...
  template: `
    <section class="page-head">
      ...
      <div style="display:flex;gap:10px"><a routerLink="/payments/record" class="ghost-button">Add Payment</a><button type="button" class="ghost-button refresh-btn" (click)="load()">
...
export class PaymentsComponent implements OnInit {
  ...
  confirmReject() {
    ...
    this.rejecting = true;
    this.rejectError = '';
    const action = this.rejectingPayment.status === 1
      ? this.api.reversePayment(this.rejectingPayment.id, this.rejectReason.trim())
      : this.api.rejectPayment(this.rejectingPayment.id, this.rejectReason.trim());
    action.subscribe({
      next: () => {
